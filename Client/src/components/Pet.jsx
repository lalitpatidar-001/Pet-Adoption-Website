...
useEffect(()=>{
  async function getIslike (userId,_id){
   try{
    const response = await axios.get(`http://localhost:4000/api/post/islike/${userId._id}?postId=${_id}`);
...
const handleLikeButton =async (e)=>{
  try{
    const response = await axios.post(`http://localhost:4000/api/post/like/${userId._id}?postId=${_id}`);
...
const handleMoreOptionClick =()=>{
  ...
  return (
    ...
    <div className='w-full h-[85vh]  flex flex-col   p-1 shadow-lg bg-white border-1 border-[#bfbdbd]'>
      ...
      <Link to={`/pet/${_id}`}>
        <div className='h-[60vh] w-[100%]   ' style={{background:`url(${imageURL})`,
